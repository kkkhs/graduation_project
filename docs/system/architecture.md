...
1. Web 前端（`frontend/`）或 CLI 工具（`tools/run_predict.py` / `tools/visualize_predict.py`）接收输入。
...
| 层             | 目录/文件                 | 可依赖对象                                    | 禁止事项                                |
| -------------- | ------------------------- | --------------------------------------------- | --------------------------------------- |
| Presentation   | `frontend/`、`tools/*.py` | `application`、`infrastructure.visualization` | 不得直接写模型加载与推理逻辑            |
