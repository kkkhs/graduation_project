# 中期答辩材料（2026-03-15 口径）

## 文件清单
- `midterm_defense_20260315.pptx`：中期答辩PPT（基于学校模板填充）
- `midterm_demo_script_20260315.md`：3-5分钟系统现场演示脚本
- `NoteGPT_Translator_2022_jianqi_chen_a.pdf`：DRENet论文翻译版PDF（中期使用）
- `pdf2zh_out_compare/2022_jianqi_chen_a-dual.pdf`：中英对照PDF（Compare PDF）
- `pdf2zh_out_compare/2022_jianqi_chen_a-zh.pdf`：纯中文翻译PDF

## 建议使用顺序
1. 先按讲稿过一遍每页口播。
2. 按演示脚本提前启动服务并走一遍闭环。
3. 把PPT封面中的学号、班级、导师补全为最终信息。
4. 若现场环境波动，按演示脚本中的截图兜底方案切换。

## 口径说明
- 时间口径固定为：截至 2026-03-15。
- 中期重点：系统完成情况、代码量、已接入 1-2 个算法的可演示闭环。
