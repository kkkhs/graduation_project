...
表~\ref{tab:ch4_baselines} 与图~\ref{fig:ch4_main_comparison_chart} 中，YOLO26 和 DRENet 的 AP50 几乎相同，分别为 0.7950 和 0.7949，FCOS 为 0.7389。仅从 AP50 看，专项方法和成熟 one-stage 路线在该数据集上都能完成较稳定检出，FCOS 与前两者仍有明显差距。
...
FCOS 的 AP50 为 0.7389，低于 DRENet 与 YOLO26；但其 Precision 为 0.7621、Recall 为 0.8297、F1 为 0.7944，说明在当前评测配置下，FCOS 对正样本保留较积极，Precision 与 Recall 的折中表现优于 DRENet。不过，FCOS 的参数量（32.17M）和计算量（123.0G）均明显高于另外两个模型，推理速度也最慢（45.96 FPS），整体效率偏低。在遥感微小船舶任务中，FCOS 的检出能力不如 DRENet 和 YOLO26，但 Precision--Recall 平衡和 F1 稳定性仍有参考价值。
