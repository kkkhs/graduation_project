...
系统部分围绕遥感微小船舶检测任务展开，目标是实现图像上传、模型推理、任务管理、结果可视化和历史记录查询等功能。系统需要覆盖前端交互、后端服务、数据持久化和算法接入几个环节，使实验结果能够以任务形式进行管理和复查。

最终方案采用 React + FastAPI + SQLite 的前后端分离架构。React 负责任务提交、状态查询和结果展示；FastAPI 组织 HTTP 接口并调用模型推理代码；SQLite 保存任务、结果和产物路径。该组合结构清晰，便于接口维护和结果追踪。
...
系统采用前后端分离架构，如图~\ref{fig:ch5_architecture} 所示。前端处理任务创建、列表查询和结果可视化；后端提供接口服务、任务调度、状态管理和静态文件访问；数据库保存模型、任务、检测结果和输出文件路径；算法能力层封装模型预测服务、模型适配器与融合逻辑，并由后端通过统一接口调用。
...
接口设计围绕任务提交、任务查询、结果查询、模型查询和健康检查展开。各接口路径统一挂载在 \texttt{/api/v1} 下，便于前端集中调用，也便于后续维护。

\subsection{关键实现流程}

\subsubsection{任务创建与执行流程}

任务从前端上传图像开始。用户选择模式、模型和阈值后，前端调用 \path{/api/v1/tasks/infer}。后端先校验图像、阈值和模型配置，再创建任务记录并写入数据库。之后任务进入线程池执行器，由执行器根据参数选择单模型或融合模式，并调用统一推理接口完成检测。
...
相比只展示一张检测结果图，Web 前端可以同时放置任务参数、执行状态、结果图像和结构化检测框。这样，实验结论不只停留在表格和文字中，也能通过系统任务记录进行复查。
...
对应改动也按这几处展开：融合推理增加可配置并行；前端轮询改为 progress 接口；可视化阶段直接传递 \texttt{PIL.Image} 对象；批量任务改为分段提交；数据集信息和参考框加入内存缓存；输入文件优先使用符号链接；应用启动时预加载启用模型。这些改动的共同目标是减少端到端流程中的等待时间。
...
此外，系统在 \texttt{startup} 阶段预加载已启用模型权重，避免第一次提交任务时才初始化模型。这个改动不改变稳态吞吐量，但能减少首次推理的额外等待。
...
为此，后端新增进度接口，只返回 \texttt{status}、\texttt{done\_count} 和 \texttt{input\_count} 等字段。前端改为两阶段轮询：任务处于 \texttt{running} 时，每 1.5 秒请求 progress 接口，只更新进度条和状态；任务转为 \texttt{done} 或 \texttt{failed} 后，再请求结果接口加载完整检测数据和可视化产物。高频轮询响应由约 25KB 缩减到约 64B，页面感知延迟也随之降低。
...
表~\ref{tab:ch5_perf} 中的收益主要来自两类变化：一是减少串行等待，例如融合推理并行和 progress 轮询；二是减少重复工作，例如图像传递优化、结果缓存和分段提交。这些结果与实际联调中的页面响应改善方向一致，说明优化思路是有效的。
